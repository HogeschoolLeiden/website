...
public interface PublicImagesInfo extends ContentBeanPathInfo {
    ...
    @Override
    @JcrPath(isRelative = false, pickerInitialPath = WidgetConstants.INITIAL_IMAGE_FOLDER_LOCATION, pickerRemembersLastVisited = false, pickerSelectableNodeTypes = { WidgetConstants.IMAGE_FOLDER_SELECTABLE })
    @Parameter(name = HstParametersConstants.CONTENT_BEAN_PATH, displayName = "Beeldbank openbaar map", defaultValue = WidgetConstants.INITIAL_IMAGE_FOLDER_LOCATION)
    public String getContentBeanPath();

    @JcrPath(isRelative = false, pickerInitialPath = WidgetConstants.ROOT_IMAGE_FOLDER, pickerRemembersLastVisited = false, pickerSelectableNodeTypes = { HippoNodeTypes.HSL_IMAGE_SET })
