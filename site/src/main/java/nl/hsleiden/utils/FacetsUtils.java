...
public class FacetsUtils {
    ...
    private static final String VAKGEBIEDEN = "Vakgebieden";
    private static final String SPECIALISATIES = "Specialisaties";
    ...
    private static String getJcrPrimaryType(String property) {
        ...
        if (filterProperty.startsWith(JCR_PRIMARY_TYPE_EQ)) {
            result = filterProperty.substring(filterProperty.indexOf(JCR_PRIMARY_TYPE_EQ)
                    + JCR_PRIMARY_TYPE_EQ.length());
        }
        return result;
    }

    public static Boolean shouldFilterGroupDisplay(HippoFacetNavigationBean facetBean) {

        boolean result = true;

        if (SPECIALISATIES.equalsIgnoreCase(facetBean.getName())
                && !facetBean.getPath().contains(PATH_SEPARATOR + VAKGEBIEDEN + PATH_SEPARATOR)) {
            result = false;
        }

        if (VAKGEBIEDEN.equalsIgnoreCase(facetBean.getName())
                && facetBean.getPath().contains(PATH_SEPARATOR + SPECIALISATIES + PATH_SEPARATOR)) {
            result = false;
        }
        return result;
    }
