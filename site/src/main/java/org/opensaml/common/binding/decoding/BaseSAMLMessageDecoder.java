...
/**
 * This class overrides default!!!! fix by Marijan from Hippo...
 * 
...
public abstract class BaseSAMLMessageDecoder extends BaseMessageDecoder implements SAMLMessageDecoder {
     ...
     */
    public BaseSAMLMessageDecoder(final ParserPool pool) {
        super(pool);
        setURIComparator(new BasicURLComparator());
    }
     ...
     */
    public void setURIComparator(final URIComparator comparator) {
        if (comparator == null) {
     ...
     * @throws org.opensaml.ws.message.decoder.MessageDecodingException thrown if the message is not an instance of SAML message that
     ...
     * <p/>
     * <p>This default implementation assumes an underlying message context {@link org.opensaml.ws.transport.InTransport} type
     * of {@link org.opensaml.ws.transport.http.HttpServletRequestAdapter} and returns the string representation of the underlying
     * request URL as constructed via {@link javax.servlet.http.HttpServletRequest#getRequestURL()}.</p>
     * <p/>
     ...
     * @throws org.opensaml.ws.message.decoder.MessageDecodingException thrown if the endpoint can not be extracted from the message
     *                                  context and converted to a string representation
     */
    protected String getActualReceiverEndpointURI(final SAMLMessageContext messageContext) throws MessageDecodingException {
     ...
     * In this case, see also {@link #getActualReceiverEndpointURI(org.opensaml.common.binding.SAMLMessageContext)}.</p>
     *
     ...
     * @throws org.opensaml.ws.message.decoder.MessageDecodingException thrown if the endpoints specified are not equivalent
     */
    protected boolean compareEndpointURIs(final String messageDestination, String receiverEndpoint)
            throws MessageDecodingException {

        final boolean isSame = getURIComparator().compare(messageDestination, receiverEndpoint);
        if(isSame){
            return true;
        }
        ...
        receiverEndpoint = CONTEXT_PATTERN.matcher(receiverEndpoint).replaceFirst("/");
     ...
     *
     * @param messageContext current message context
     * @throws org.opensaml.xml.security.SecurityException        thrown if the message Destination attribute is invalid
     *                                  with respect to the receiver's endpoint
     * @throws org.opensaml.ws.message.decoder.MessageDecodingException thrown if there is a problem decoding and processing
     *                                  the message Destination or receiver
     *                                  endpoint information
     */
    protected void checkEndpointURI(final SAMLMessageContext messageContext)
