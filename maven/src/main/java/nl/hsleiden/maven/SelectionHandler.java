...
public class SelectionHandler extends ContentTypeItemHandler {
	...
	public SelectionHandler(Map<String, HippoBeanClass> beansOnClassPath,
			Map<String, HippoBeanClass> beansInProject, Set<String> namespaces,
		...
		if ("DynamicDropdown".equals(item.getType())) {
			...
			if (template != null) {
				...
				if (pathToValueList != null) {
					ClassReference reutrnType = new ClassReference(
							SelectionBean.class);
					importRegistry.register(reutrnType);
					List<PropertyGenerator> propertyGenerators = Collections
							.singletonList((PropertyGenerator) new DefaultPropertyGenerator(
									new AnalyzerResult(Type.PROPERTY,
											reutrnType), item, importRegistry));
					List<MethodGenerator> methodGenerators = Collections
							.singletonList((MethodGenerator) new SelectionMethodGenerator(
									item, pathToValueList, reutrnType));

					result = new HandlerResponse(propertyGenerators,
							methodGenerators);
